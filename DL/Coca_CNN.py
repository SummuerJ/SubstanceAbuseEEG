...
def load_dataset(path):
    ...
    train_set_x_orig = dataa[:27024]  # your train set features  # 5405  # 27024
...
with graph.as_default():
    ...
    # (batch, 100, 20) --> (batch, 50, 40)
    conv2 = tf.layers.conv1d(inputs=max_pool_1, filters=40, kernel_size=2, strides=1,
                             padding='same', activation=tf.nn.relu)
    max_pool_2 = tf.layers.max_pooling1d(inputs=conv2, pool_size=2, strides=2, padding='same')

    # (batch, 32, 36) --> (batch, 16, 72)
    # (batch, 50, 40) --> (batch, 25, 80)
    conv3 = tf.layers.conv1d(inputs=max_pool_2, filters=80, kernel_size=2, strides=1,
                             padding='same', activation=tf.nn.relu)
    max_pool_3 = tf.layers.max_pooling1d(inputs=conv3, pool_size=2, strides=5, padding='same')

    # (batch, 16, 72) --> (batch, 8, 144)
    # (batch, 25, 80) --> (batch, 5, 400)
    conv4 = tf.layers.conv1d(inputs=max_pool_3, filters=400, kernel_size=2, strides=1,
                             padding='same', activation=tf.nn.relu)
    max_pool_4 = tf.layers.max_pooling1d(inputs=conv4, pool_size=2, strides=2, padding='same')

with graph.as_default():
    # Flatten and add dropout
    flat = tf.reshape(max_pool_4, (-1, 5 * 400))
