...
for i in range(7):  # 7只小鼠
    ...
    for j in range(0, 5):  # 降采样后的每一条都可以用作数据
        ...
        while k + 6000 <= 108000:  # 108000:    # 采取30s判断
            ...
            gasf1 = GramianAngularField(image_size=120, method='summation')  # Transform the time series into Gramian Angular Fields
            gasf2 = GramianAngularField(image_size=120, method='summation')
            gasf3 = GramianAngularField(image_size=120, method='summation')
    ...
    for j in range(0, 5):  # 降采样后的每一条都可以用作数据
        ...
        while k + 6000 <= 108000:  # 108000:    # 采取30s判断
            ...
            gasf1 = GramianAngularField(image_size=120,
                                        method='summation')  # Transform the time series into Gramian Angular Fields
            gasf2 = GramianAngularField(image_size=120, method='summation')
            gasf3 = GramianAngularField(image_size=120, method='summation')
            X_gasf1 = gasf1.fit_transform(activity_list1)
            X_gasf2 = gasf2.fit_transform(mobility_list1)
