...
怎么读取需要你自己写
'''
coca_data = np.load("psd_coca_20210511.npy")  # (10975, 50)
sali_data = np.load("psd_sali_20210511.npy")  # 18050,50
...
for sali_D in sali_data:
    ...
    gasf = GramianAngularField(image_size=50, method='difference')  # Transform the time series into Gramian Angular Fields
    X_gasf = gasf.fit_transform(data)
    # coca_im_total.append(X_gasf[0])
    # print(X_gasf[0])
    X_im = ((X_gasf[0] + 1) / 2) * 255
...
for coca_D in coca_data:
    ...
    gasf = GramianAngularField(image_size=50, method='difference')  # Transform the time series into Gramian Angular Fields
    X_gasf = gasf.fit_transform(data2)
    # coca_im_total.append(X_gasf[0])
    # print(X_gasf[0])
    X_im = ((X_gasf[0] + 1) / 2) * 255
    X_im = X_im.astype(np.uint8)
...
save_image_to_h5py('20210510_Dataset_GADF_50x50x3_21494.h5')
