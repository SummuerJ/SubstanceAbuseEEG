...
for i in range(7):  # 7只小鼠
    ...
    for j in range(0, 5):  # 降采样后的每一条都可以用作数据
        ...
        while k + 6000 <= 108000:  # 108000:    # 采取30s判断
            ...
            gasf1 = GramianAngularField(image_size=120, method='difference')  # Transform the time series into Gramian Angular Fields
            gasf2 = GramianAngularField(image_size=120, method='difference')
            gasf3 = GramianAngularField(image_size=120, method='difference')
            X_gasf1 = gasf1.fit_transform(activity_list1)
            X_gasf2 = gasf2.fit_transform(mobility_list1)
            X_gasf3 = gasf3.fit_transform(complexity_list1)

            X_im1 = ((X_gasf1[0] + 1) / 2) * 255
            X_im2 = ((X_gasf2[0] + 1) / 2) * 255
            X_im3 = ((X_gasf3[0] + 1) / 2) * 255
...
for i in range(7):  # 7只小鼠
    ...
    for j in range(0, 5):  # 降采样后的每一条都可以用作数据
        ...
        while k + 6000 <= 108000:  # 108000:    # 采取30s判断
            ...
            gasf1 = GramianAngularField(image_size=120,
                                        method='difference')  # Transform the time series into Gramian Angular Fields
            gasf2 = GramianAngularField(image_size=120, method='difference')
            gasf3 = GramianAngularField(image_size=120, method='difference')
            X_gasf1 = gasf1.fit_transform(activity_list1)
            X_gasf2 = gasf2.fit_transform(mobility_list1)
...
# save_image_to_h5py('20210410_Dataset_hjorth_GADF_120x120x3.h5')